...
- **多纪念日轮播**：支持多个纪念日，左右滑动/拖拽切换，底部圆点指示器
- 天数大字渐变 + 时分秒实时流动
- ISTP ♥ ISFJ 个性化展示
- **特殊天数横幅**：100/365/520/978/1000/1314 天等里程碑提醒
- 支持精确到时分的纪念日时间（如 2023.10.08 01:03）

### 🌍 旅行地图

一起走过的城市，标记在地图上，按年份回顾。

- **Leaflet 地图**：30+ 座城市标记，默认暗色瓦片，支持暗色/亮色一键切换
- **双向联动**：点地图标记 ↔ 时间线滚动高亮
- **年份筛选**：按 2023/2024/2025/2026 筛选，地图视野自动适配
- **时间线**：按年份分组，点击飞入城市 + 弹窗详情
- **桌面自适应**：旅行页撑满全宽，移动端保持紧凑
- **数据可编辑**：在数据管理页面增删改旅行记录
...
```bash
git add -A                          # 暂存所有修改
git commit -m "feat: 描述你的改动"    # 提交到本地
git push origin main                # 推送到 GitHub 库 main分支
```


推荐使用 [约定式提交](https://www.conventionalcommits.org/zh-hans/) 格式：
- `feat:` 新功能
- `fix:` 修复 bug
- `docs:` 文档更新
- `style:` 样式调整
...
推送 `main` 分支后 GitHub Actions 自动构建部署到 GitHub Pages。稍候几分钟即可刷新页面。
...
│   ├── components/
│   │   ├── decider.js             # 随机决策器
│   │   ├── anniversary.js         # 纪念日页面（多卡片轮播）
│   │   ├── jar.js                 # 默契储蓄罐
│   │   ├── quests.js              # 协作清单
│   │   ├── travel.js              # 旅行地图（Leaflet）
│   │   └── settings.js            # 数据管理 + 主题切换 + 纪念日/旅行记录编辑
│   ├── utils/
│   │   ├── helpers.js             # 数据源 + 工具函数 + 纪念日/旅行数据
